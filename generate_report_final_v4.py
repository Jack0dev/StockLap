...
def run():
    ...
    ws.cell(row=2, column=data_col_2, value=298)
    ws.cell(row=3, column=data_col_1, value="Fail")
    ws.cell(row=3, column=data_col_2, value=12)
    ...
    ws.cell(row=6, column=data_col_2, value=305)
    ...
    # ==========================================
    manual_data = [
        ("Tổng số Test Case", 310, "-", "Bao phủ các Module toàn hệ thống"),
        ("Đã thực hiện", 310, "100%", ""),
        ("Test Pass", 298, "96.1%", "Đạt yêu cầu logic mượt mà"),
        ("Test Fail / Bug", 12, "3.9%", "Lỗi UI/UX & validation nhẹ"),
        ("Chưa thực hiện", 0, "0%", "")
    ]
    ...
    # Chart 1
    pie1 = PieChart()
    pie1.title = "Manual Testing (310 TCs)"
    labels1 = Reference(ws, min_col=data_col_1, min_row=2, max_row=3)
    data1 = Reference(ws, min_col=data_col_2, min_row=1, max_row=3)
    ...
    # ==========================================
    api_data = [
        ("Tổng số API Functional (Postman)", 170, "-", "StockLab_API Collection"),
        ("Tổng số test Security", 140, "-", "Kiểm tra lỗ hổng bảo mật tiêu chuẩn OWASP"),
        ("Request Pass (Thành công/Chặn an toàn)", 305, "98.4%", "Đạt yêu cầu"),
        ("Request Fail (Lỗi/Lọt lỗ hổng)", 5, "1.6%", "Lỗi validation input nhẹ"),
    ...
    ws.cell(row=current_row, column=2, value="Chi tiết Security (140 TCs):").font = Font(bold=True, italic=True)
    current_row += 1
    
    # Add Security Mini Table
    sec_data = [
        ("SQL Injection", 38, "Chặn 100%"),
        ("XSS Attack", 40, "Chặn 100%"),
        ("Path Traversal", 46, "Chặn 100%"),
        ("Unauthorized Access", 16, "Chặn 100%"),
    ]
    ...
    # Chart 2
    pie2 = PieChart()
    pie2.title = "API & Security Testing (310 TCs)"
    ...
    # ==========================================
    jmeter_data = [
        ("Số kịch bản tự động (Test Cases)", 310, "-", "Theo chuẩn IEEE Test Suite"),
    ...
    # ==========================================
    overall_data = [
        ("Tổng mức độ hoàn thiện kiểm thử", "PASS", "100%", "Đã kiểm thử toàn vẹn 310 cases"),
        ("Chất lượng logic hệ thống (Core logic)", "Tốt", "98.4%", "Logic matching / auth hoạt động cực tốt"),
